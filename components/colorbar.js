// colorbar.js is taken from carbonplan's components repo and then modified
...
const Colorbar = ({
  ...
  const draggingFunction = (e) => {
    ...
    } else { {/* section for Dif., changes both color ranges at the same time  */}
      if (id === 'min')
        setClim((prev) => [Math.min(init[0] - dy * scale, 0), Math.max(init[1] + dy * scale, 0)])
      if (id === 'max')
        setClim((prev) => [Math.min(init[0] + dy * scale, 0), Math.max(init[1] - dy * scale, 0)])
  ...
  const ClimMin = () => {
    return (
      ...
      >
        {format(clim[0].toFixed(fixedVal))}
      </Box>
    )
  }
  ...
  const ClimMax = () => {
    return (
      ...
      >
        {format(clim[1].toFixed(fixedVal))}
      </Box>
    )
  }
