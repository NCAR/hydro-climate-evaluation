...
const ParameterControls = ({ getters, setters, bucket, fname, settings }) => {
  ...
  const ensembleExists = async (
    ...
    const time = getYearRangeString(yearRange);
    const url =
      `${bucket}${baseDir}${downscaling}/${model}/` +
      `${time}/${ensemble}/${fname}/.zmetadata`;
  ...
  useEffect(() => {
    ...
    async function updateAvailableEnsembles() {
      ...
      const candidates = settings.ensemble?.[downscaling]?.[model] ?? [];
      ...
      setAvailableEnsembles(available);

      if (available.length > 0) {
        const safeEnsemble = available.includes(ensemble)
          ? ensemble
          : available[0];

        if (safeEnsemble !== ensemble) {
          setEnsemble(safeEnsemble);
        }
        setUrl(baseDir, downscaling, model, yearRange, safeEnsemble);
      }
    }

    updateAvailableEnsembles();
  ...
  const YearRangeBox = ({downscaling_l, past = true, future = true,
    ...
    return(
      <>
      <Box sx={{ ...sx.label, mt: [3] }}>Year Range/Scenario</Box>
      <Select
          sxSelect={{ bg: 'transparent' }}
          size='xs'
          onChange={handle}
